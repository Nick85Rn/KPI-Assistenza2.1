...
import {
  getTicketKpis, getChatKpis, getFormazioneKpis, getLastSyncByPart,
  getChatHeatmap, getTopVisitors, getFormazioneDetails,
  getAssistenzaDetails, getSviluppoDetails, getChatAnalysisData,
} from "../api/zohoData";

const BACKLOG_POLL_INTERVAL_MS = 120000;
...
export function useDashboardData({
  ...
  const [state, setState] = useState({
    loading: true, error: null, current: null, previous: null, yoy: null,
    lastSync: null, heatmap: null, topVisitors: null, formazioneDetails: null,
    assistenzaDetails: null, sviluppoDetails: null, chatAnalysis: null,
  });
  ...
  const extrasKey = JSON.stringify({
    heatmap: !!extras.heatmap, topVisitors: !!extras.topVisitors,
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        lastSync, heatmapData, topVisitorsData,
        formazioneDetailsData, assistenzaDetailsData,
        sviluppoDetailsData, chatAnalysisData,
      ] = await Promise.all([
      ...
      setState({
        ...
        lastSync, heatmap: heatmapData, topVisitors: topVisitorsData,
  ...
  }, [start?.getTime(), end?.getTime(), extras.assistenzaDetails, extras.sviluppoDetails]);

  useEffect(() => { load(); }, [load]);

  useEffect(() => {
    if (!extras.assistenzaDetails && !extras.sviluppoDetails) return;
    if (backlogIntervalRef.current) clearInterval(backlogIntervalRef.current);
    backlogIntervalRef.current = setInterval(() => {
      if (document.visibilityState === "visible") refreshBacklog();
    }, BACKLOG_POLL_INTERVAL_MS);
    return () => { if (backlogIntervalRef.current) clearInterval(backlogIntervalRef.current); };
