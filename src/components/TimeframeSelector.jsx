...
export default function TimeframeSelector({ type, anchor, onChange }) {
  ...
  const setType = (newType) => {
    // Quando si cambia tipo, riportiamo l'ancora a oggi per coerenza
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      {/* Pulsante "Oggi" — appare solo se siamo in un periodo non corrente */}
      {!isCurrent && (
        <button
          onClick={goToToday}
...
/**
 * True se l'ancora corrente coincide con quella di "oggi" (per il tipo dato).
 * Es. type=week, anchor=lunedì di questa settimana -> true.
 */
function isAnchorCurrent(type, anchor) {
  ...
  switch (type) {
    case "week": {
      // Confronto su numero settimana ISO + anno
...
function isoWeekKey(d) {
  ...
  // Approssimazione settimana ISO: primo giorno = lunedì
