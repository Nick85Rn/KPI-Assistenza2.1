...
import { X, ExternalLink, MessageSquare, AlertTriangle, ThumbsDown, ThumbsUp, Minus, Clock, User } from "lucide-react";
...
export default function ChatDrillDownModal({
  ...
  async function loadPage(newOffset, replace = false) {
    setLoading(true);
    try {
      const { rows: newRows, totalCount: count } = await getChatsByCategory({
        category, subcategory, period,
        sentimentFilter, resolvedFilter,
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-slate-200 flex items-start justify-between gap-4 sticky top-0 bg-white rounded-t-xl">
          <div className="min-w-0">
            ...
            <h2 className="text-lg font-bold text-slate-900 truncate">
              {subcategory || `Tutte le chat ${category}`}
            </h2>
            ...
            <div className="text-sm text-slate-600 mt-2">
              ...
              {rows.length > 0 && rows.length < totalCount && (
                <span className="text-slate-400"> (mostrate {formatNumber(rows.length)})</span>
              )}
            </div>
          </div>
          <button
        ...
        <div className="px-6 py-3 border-b border-slate-200 bg-slate-50 flex flex-wrap items-center gap-2">
          ...
          {[null, "urgente", "negativo", "neutro", "positivo"].map((s) => (
            <button
              key={s ?? "all"}
            ...
            >
              {s ? SENTIMENT_META[s]?.label : "Tutti"}
            </button>
          ))}
                ...
                {rows.map((c) => {
                  ...
                  return (
                    <tr key={c.chat_id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-center">
                        {c.resolved === true && (
                          <span className="inline-block px-2 py-0.5 rounded text-[11px] font-medium bg-emerald-100 text-emerald-700">Sì</span>
                        )}
                        {c.resolved === false && (
                      ...
                      <td className="px-4 py-3 text-right">
                          ...
                          href={`${ZOHO_SALESIQ_BASE}/${c.chat_id}`}
                          target="_blank"
