...
export default function Cruscotto({ data }) {
  const { loading, error, current, previous, yoy } = data;
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <KPICard
            ...
            value={cur.assistenza?.new_tickets}
          ...
          <KPICard
            ...
            value={cur.sviluppo?.new_tickets}
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <KPICard
            ...
            value={cur.assistenza?.closed_tickets}
          ...
          <KPICard
            ...
            value={cur.sviluppo?.closed_tickets}
      ...
      {cur.chat?.operators?.length > 0 && (
        <section>
          <SectionTitle
            title="Top operatori chat"
            hint="Ordinati per numero di chat gestite nel periodo"
          />
          <OperatorTable operators={cur.chat.operators.slice(0, 8)} />
        </section>
      )}

      {/* Sezione: Top operatori formazione */}
      {cur.formazione?.operators?.length > 0 && (
        <section>
          <SectionTitle
            title="Top formatori"
            hint="Operatori con più ore di formazione registrate"
          />
          <FormazioneTable operators={cur.formazione.operators.slice(0, 8)} />
        </section>
      )}
    </div>
  );
...
// =============================================================
// Sotto-componenti privati (li sposterò in file separati nelle fasi future)
// =============================================================
    ...
    <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-100">
          {operators.map((op) => (
            <tr key={op.operator} className="hover:bg-slate-50 transition-colors">
              <td className="px-4 py-3 font-medium text-slate-900">{op.operator}</td>
              <td className="px-4 py-3 text-right tabular-nums">{formatNumber(op.chats)}</td>
              <td className="px-4 py-3 text-right tabular-nums text-slate-600">
                {formatNumber(op.attended)}
              </td>
              <td className="px-4 py-3 text-right tabular-nums text-slate-600">
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-100">
          {operators.map((op) => (
            <tr key={op.operator} className="hover:bg-slate-50 transition-colors">
              <td className="px-4 py-3 font-medium text-slate-900">{op.operator}</td>
              <td className="px-4 py-3 text-right tabular-nums">{formatNumber(op.count)}</td>
              <td className="px-4 py-3 text-right tabular-nums text-slate-600">
                {formatMinutes(op.minutes)}
              </td>
              <td className="px-4 py-3 text-right tabular-nums text-slate-600">
                {op.count > 0 ? formatMinutes(Math.round(op.minutes / op.count)) : "—"}
              </td>
            </tr>
          ))}
        </tbody>
